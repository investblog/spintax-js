...
Status: IMPLEMENTED — `@spintax/core` **0.3.2** is published to npm (MIT, provenance via OIDC).
...
> long ago, and the package is on **0.3.2**. The only open milestone is **M6** (browser playground);
